...
@torch.no_grad()
def rw_sequential(model, dataloader, dev):
    print("Starting ...")
...
def rw_pack(model, quantizers, wbits, groupsize):
...
if __name__ == "__main__":
    ...
    parser.add_argument("model", type=str, help="rw model to load")
    parser.add_argument(
        "dataset",
        type=str,
    ...
    if not args.load and args.wbits < 16 and not args.nearest:
        tick = time.time()
        quantizers = rw_sequential(model, dataloader, DEV)
        print(time.time() - tick)

    if args.benchmark:
        gpus = [torch.device("cuda:%d" % i) for i in range(torch.cuda.device_count())]
        if len(gpus) > 1:
            rw_multigpu(model, gpus)
        else:
            model = model.to(DEV)
        if args.benchmark:
    ...
    if args.eval:
        ...
        for dataset in datasets:
            ...
            print(dataset)
            rw_eval(model, testloader, DEV)

    if args.save:
        rw_pack(model, quantizers, args.wbits, args.groupsize)
